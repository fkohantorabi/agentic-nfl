...
class Gemini(Model):
    ...
    Rate Limiting:
    - Set `rate_limit` to specify the maximum number of requests per minute.
    - When rate limiting is enabled, requests will be delayed if necessary to stay within the specified limit.
    - Rate limiting works for both synchronous and asynchronous API calls.
    - If `rate_limit` is None (default), no rate limiting is applied.
    ...
    rate_limit: Optional[int] = None  # Maximum requests per minute
    ...
    def get_client(self) -> GeminiClient:
        ...
        if self._rate_limiter is None:
            self._rate_limiter = RateLimiter(requests_per_minute=self.rate_limit)
            if self.rate_limit:
                log_info(f"Rate limiting enabled: {self.rate_limit} requests per minute")
    ...
    def invoke(
        ...
        if self._rate_limiter:
            ...
            if wait_time > 0:
                log_debug(f"Rate limit applied: waited {wait_time:.2f}s before request")
    ...
    def invoke_stream(
        ...
        if self._rate_limiter:
            wait_time = self._rate_limiter.wait_if_needed()
            if wait_time > 0:
                log_debug(f"Rate limit applied: waited {wait_time:.2f}s before stream request")
    ...
    async def ainvoke(
        ...
        # Apply rate limiting if configured
        if self._rate_limiter:
            wait_time = await self._rate_limiter.async_wait_if_needed()
            if wait_time > 0:
                log_debug(f"Rate limit applied: waited {wait_time:.2f}s before async request")
    ...
    async def ainvoke_stream(
        ...
        if self._rate_limiter:
            wait_time = await self._rate_limiter.async_wait_if_needed()
            if wait_time > 0:
                log_debug(f"Rate limit applied: waited {wait_time:.2f}s before async stream request")
